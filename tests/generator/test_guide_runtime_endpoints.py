...
def test_ghost_printer_is_session_only_and_never_persisted(
    ...
    selected = client.put(
        "/api/printers/active",
        json={"active_printer_id": "tutorial-printer", "active_nozzle_size": 0.4},
        headers=headers,
    )
    assert selected.status_code == 200
    assert selected.json()["printer"]["id"] == "tutorial-printer"
    assert selected.json()["nozzle"]["size"] == 0.4
...
def test_server_snapshot_restores_printer_nozzle_modules_and_settings_exactly(
    tmp_path: Path, monkeypatch
) -> None:
    printers_path = tmp_path / "printers.json"
    ...
    try:
        ...
        printers["active_nozzle_size"] = 0.4
        ...
        server.session["config"]["solver_fine_pitch_mm"] = 0.123
        snapshot = server._capture_guide_server_snapshot()

        changed = server._load_printers()
        changed["active_nozzle_size"] = 0.2
        ...
        server.session["config"]["solver_fine_pitch_mm"] = 0.4
        ...
        assert restored_printers["active_nozzle_size"] == 0.4
        assert server.load_module_state(modules_path) == snapshot["modules"]
        assert server.session["config"]["solver_fine_pitch_mm"] == 0.123
        assert server.session["config"]["image_path"] is None
        assert server.session["config"]["palette"] == []
    finally:
        server.session.clear()
