...
def test_session_config_rejects_pitch_without_an_in_range_grid():
    response = TestClient(server.app).post(
        "/api/session/config",
        json={
            "frame": {"width_mm": 100.0, "height_mm": 100.0},
            "image_sample_pitch_mm": 301.0,
            "solver_fine_pitch_mm": 301.0,
        },
    )

    assert response.status_code == 422
    assert response.json()["detail"]["error"] == "invalid_solve_grid"
