# lithophane_generator/tests/test_facade_results.py
...
def test_solve_full_returns_solve_result():
    img = np.random.default_rng(2).integers(0, 256, (8, 8, 3), dtype=np.uint8)
...
def test_solve_full_progress_callback():
    progress_calls = []
    img = np.random.default_rng(4).integers(0, 256, (4, 4, 3), dtype=np.uint8)
