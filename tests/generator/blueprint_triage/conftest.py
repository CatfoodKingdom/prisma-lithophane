...
from pipeline.state import PipelineConfig, PipelineState, PREVIEW_PRESET
...
def make_state(
    ...
    *,
    ...
    state = PipelineState(
        image=np.zeros((*plan.shape, 3), dtype=np.uint8) if image is None else np.asarray(image, dtype=np.uint8),
        config=cfg,
        solved_plan=plan,
    )
    ...
    state.diagnostics = {
        "__de__": (
            np.zeros(plan.shape, dtype=np.float32)
            if de_map is None
            else np.asarray(de_map, dtype=np.float32)
        ),
...
@pytest.fixture
def sample_plan() -> SolvedMaterialPlan:
    return make_plan()


@pytest.fixture
def sample_state(sample_plan: SolvedMaterialPlan) -> PipelineState:
    return make_state(sample_plan)
...
def make_oklab_target(
    shape: tuple[int, int],
    *,
    default_l: float = 0.45,
    highlight_mask: np.ndarray | None = None,
    highlight_l: float = 0.80,
) -> np.ndarray:
    target = np.full((*shape, 3), (default_l, 0.0, 0.0), dtype=np.float32)
    if highlight_mask is not None:
        target[np.asarray(highlight_mask, dtype=bool), 0] = float(highlight_l)
