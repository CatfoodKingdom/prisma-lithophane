import { chapter, completion, detour, step } from "../../core/schema.js";
...
const BASICS_LESSONS = Object.freeze([
  ...
  step({
    ...
    body: "Nozzle size controls how small features in a lithophane can be. This ensures that the finished lithophane should be printable with a given nozzle size. A 0.2 mm nozzle will permit finer details than a 0.4 mm nozzle, but it will also require more time to print. Prisma’s calculations also take noticeably longer with a 0.2 mm nozzle.\n\nThis guide uses a 0.4 mm nozzle to save time while you learn the basics. Set the nozzle size to 0.4 mm using the drop-down nozzle selector to proceed.",
  ...
  step({
    ...
    placements: [],
    visual: {
      clone_selector: '[data-guide-visual="lithophane-stack"]',
      label: "Cross-section of a lithophane showing its white base, color layers, white cap, layer height, pixel size, and maximum total thickness",
    },
  }),
  step({
    id: "open-settings",
    title: "Open Settings",
    body: "This guide uses the prepared tutorial defaults.\n\nOpen Settings to review them and continue.",
  ...
  step({
    ...
    body: "The Settings drawer shows all of the parameters that will be used the next time you select **Solve**.\n\nThese values can be adjusted at any time, but changing them does not retroactively change any solves you have already run.",
  ...
  step({
    ...
    body: "Leave **Solve Mode** set to **Color** for this guide.\n\n**Solve Pitch** is the physical spacing between points in the grid Prisma uses to plan a lithophane. A smaller value can preserve finer details, but it also increases solving time. Solve Pitch cannot be smaller than the active nozzle size.\n\nKeep the tutorial value of 0.4 mm.",
  ...
  step({
    ...
    body: "A Settings Profile saves all of the parameters in the Settings drawer so you can reuse them later.\n\n**Save** lets you update a loaded named profile or save the current settings as a new profile. System and temporary profiles cannot be overwritten. **Save As...** always creates a new profile. Use **Profiles...** to browse, load, and manage saved profiles.",
...
const BASICS_DETOURS = Object.freeze([
  ...
  detour({
    id: "settings-tools",
    label: "Settings",
    description: "Explore Settings Profiles, solver controls, White Cap, and Advanced settings without changing the tutorial values.",
