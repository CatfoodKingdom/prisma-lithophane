...
export function installFeaturesGuideActionRegistry(app) {
  ...
  const resultContracts = Object.freeze({
    ...
    "printer.select_nozzle": "active-printer",
  ...
  registerGuideAction("printer.select", {
    ...
    async run(input) {
      const active = await app.api.setActivePrinter({ active_printer_id: input.printer_id });
      await app.commands.loadPrinters();
      return active;
    },
  });
  registerGuideAction("printer.select_nozzle", {
    idempotent: true,
    workspacePolicies: ["basic-teaching"],
    validate(input) {
      if (!Number.isFinite(Number(input.nozzle_mm)) || Number(input.nozzle_mm) <= 0) {
        throw new Error("printer.select_nozzle requires a positive nozzle_mm");
      }
    },
    async run(input) {
      const active = await app.api.setActivePrinter({ active_nozzle_size: Number(input.nozzle_mm) });
      await app.commands.loadPrinters();
      return active;
    },
  });
  ...
  registerGuideAction("settings.override", {
    ...
    validate(input) {
      ...
      const knownSettings = new Set(app.state.ui.SETTINGS_PROFILE_KEYS || []);
    ...
    async run(input) {
      Object.assign(app.state.settings.config, JSON.parse(JSON.stringify(input.values)));
      ...
      return JSON.parse(JSON.stringify(input.values));
    },
  });
